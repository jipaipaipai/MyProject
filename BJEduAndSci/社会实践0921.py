...
#data = pd.read_excel(path +'数据\\' +'房山区2017-2018学年数据20180920.xlsx')#读取excel
#pd.to_pickle(data, path+'dc')
data = pd.read_pickle(path+'dc')
...
'''制作词云'''
#对活动类型分类进行词云制作
wc = WordCloud(font_path='simhei.ttf',background_color='Black', max_words=100, stopwords=STOPWORDS)
for i in range(len(name_gro)):
    wc.generate(' '.join(wordseg_gro[i]))
    plt.imshow(wc)
    plt.axis("off")
    plt.savefig(path+'活动分类词云\\'+name_gro[i]+'.png')
    plt.show()
